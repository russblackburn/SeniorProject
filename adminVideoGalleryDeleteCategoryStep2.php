...
//CHANGE LOCATION REDIRECT - CHANGE ADMINJAKE.PHP TO NEW DB
	...
	if (isset($_POST['submit'])){
		...
		// GET THE SUBCATEGORY INFORMATION FROM THE DATABASE
			//BUILD THE QUERY1
			$query1 = "SELECT * FROM videoSubcategory WHERE categoryID=$categoryID";
			
			//TRY AND TALK TO THE DB
			$result1 = mysqli_query($dbc, $query1) or die('query failed 1');
			
		// GET THE VIDEO INFORMATION FROM THE DATABASE AND DELETE IT AT THE SAME TIME
			//FOR EACH SUBCATEGORY, SELECT THE CORRESPONDING VIDEOS OF THAT SUBCATEGORY AND DELETE THEM
			while($row1 = mysqli_fetch_array($result1)){
				...
				// GET THE RECORDS FROM THE DATABASE
					//BUILD THE QUERY2
					$query2 = "SELECT * FROM videoGallery WHERE subcategoryID=$subcategoryID";
					
					//TRY AND TALK TO THE DB
					$result2 = mysqli_query($dbc, $query2) or die('query failed 2');
				
				// REMOVED THE CODE TO DELETE PHOTOS----------------
				
				// DELETE THE RECORDS FROM THE DATABASE
					//BUILD A SELECT QUERY
					$query3 = "DELETE FROM videoGallery WHERE subcategoryID=$subcategoryID";
		...
		// DELETE THE CATEGORY ITEMS
			...
			//BUILD A SELECT QUERY
			$query = "DELETE FROM videoCategory WHERE id=$categoryID";
		...
		// DELETE THE SUBCATEGORY ITEMS
			// USE ANOTHER QUERY TO DELETE ALL OF THE PHOTOS IN THE SUBCATEGORY FOLDERS
				//BUILD THE QUERY1
				$query4 = "SELECT * FROM videoSubcategory WHERE categoryID=$categoryID";
			...
			while($row4 = mysqli_fetch_array($result4)){
				...
				$subcategoryPhoto = 'images/gallery/video/subcategory/'.$row4[photo];
				@unlink($subcategoryPhoto);
			}
			
			//BUILD A SELECT QUERY
			$query1 = "DELETE FROM videoSubcategory WHERE categoryID=$categoryID";
		...
		//REDIRECT
		header("Location: adminVideoGalleryDeleteCategory.php");
...
echo '<div class="row">';
echo '<img class="col-xs-12 col-sm-6 col-md-4" src="images/gallery/video/category/'.$found['photo'].'">';
echo '</div>';
...
<input type="hidden" name="photo" value="images/gallery/video/category/<?php echo $found['photo'];?>">
